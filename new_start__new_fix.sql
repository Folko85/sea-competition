...
begin
	...
	select * from world.contractors cust where  "type" = 'customer' 
			...
			AND NOT exists (SELECT * FROM custom.contracts ct WHERE ct.vendor_id = vend.id AND status IN ('ACTIVE', 'BROKEN')))
			AND NOT exists (SELECT * FROM custom.contracts ct WHERE ct.customer_id  = cust.id AND status IN ('ACTIVE', 'BROKEN'))
